...
int main() {
    ...
    Shader objectShader("../emerald-isle/objectShader.vert", "../emerald-isle/objectShader.frag");
    ...
    setup_instancing(treeVAO, tree_chunks, "tree", plants, "../emerald-isle/obj/CommonTree_1.obj");
    setup_instancing(flowerVAO, flower_chunks, "flower", plants, "../emerald-isle/obj/Flowers.obj");
...
void load_model(GLuint &VAO, std::string filename) {
    ...
    std::string basePath = "../emerald-isle/obj/";
    tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, filename.c_str(), basePath.c_str());
        ...
        for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
            ...
            for (size_t v = 0; v < fv; v++) {
                ...
                if (idx.vertex_index < 0 || idx.vertex_index >= attrib.vertices.size() / 3) {
                    std::cerr << "Invalid vertex index: " << idx.vertex_index << std::endl;
                    continue;
                }
                ...
                if (idx.normal_index >= 0 && idx.normal_index < attrib.normals.size() / 3) {
                    ...
                    vertices.push_back(0.0f); // Default normal
                    vertices.push_back(0.0f);
                    vertices.push_back(0.0f);
                }

                float r = 1.0f, g = 1.0f, b = 1.0f; // Default color
                if (shapes[s].mesh.material_ids[f] >= 0 && shapes[s].mesh.material_ids[f] < materials.size()) {
                    r = materials[shapes[s].mesh.material_ids[f]].diffuse[0] * MODEL_BRIGHTNESS;
                    g = materials[shapes[s].mesh.material_ids[f]].diffuse[1] * MODEL_BRIGHTNESS;
                    b = materials[shapes[s].mesh.material_ids[f]].diffuse[2] * MODEL_BRIGHTNESS;
                }
...
int init() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Emerald Isle", nullptr, nullptr);
